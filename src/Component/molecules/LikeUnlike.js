  ...
  const handleLike = async () => {
    if (props?.type === "likeForum") {
      if (state?.is_you_like) {
        await axios
          .delete(`http://43.218.14.145:9090/like/forum/${tempApi?.id}`, {
            headers: {
        ...
        await axios
          .post(
            `http://43.218.14.145:9090/like/forum/${tempApi?.id}`,
            {},
            {
              headers: {
                "Content-Type": "application/json",
      ...
      if (state?.is_you_like) {
        await axios
          .delete(`http://43.218.14.145:9090/like/blog/${tempApi?.id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${key}`,
            },
        ...
        await axios
          .post(
            `http://43.218.14.145:9090/like/blog/${tempApi?.id}`,
            {},
            {
              headers: {
                "Content-Type": "application/json",
